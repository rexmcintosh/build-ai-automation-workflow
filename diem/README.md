...
The drain is **read-and-stage only**. It never commits, pushes, merges, publishes, or touches KDP. Review findings land as report files; image candidates append to the repo's candidates dir; loom writes to its own store. All output sits behind the existing human gates. `cmd` runs only whitelisted commands keyed by repo. Images flagged `x-venice-is-content-violation` are quarantined and noted in the report. Standing-order files (repo-owned) declare target stock levels, candidate dir, and the run command; no standing order → no images job. This boundary keeps diem out of creative direction; open decisions (e.g., object-row direction) remain the operator's.
