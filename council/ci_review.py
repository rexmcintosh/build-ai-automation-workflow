"""Venice review council — CI entry point (`council-ci-review`) over the engine.

One shim in the package instead of 17 copies (see
docs/venice-review-shim-audit-2026-07-23.md): CI workflows run
`council-ci-review` and the existing version pin governs the shim and the
engine together, so shim drift is impossible by construction.
...
Hardening (see docs/venice-review-shim-audit-2026-07-23.md):
...
Required env: VENICE_COUNCIL_KEY or VENICE_API_KEY (via council.config.get_api_key),
              GITHUB_TOKEN, PR_NUMBER, REPO, DIFF_PATH
...
def _read_capped(path: Path, cap: int) -> str:
    """Read at most ~cap bytes of `path`: head half + tail half with the same
    truncation marker council.config.truncate produces — but WITHOUT loading the
    whole file first, so a huge changed file cannot spike CI memory."""
    size = path.stat().st_size
    if size <= cap:
        return path.read_text(encoding="utf-8", errors="ignore")
    head_n, tail_n = cap // 2, (cap + 1) // 2
    with path.open("rb") as fh:
        head = fh.read(head_n).decode("utf-8", errors="ignore")
        fh.seek(max(0, size - tail_n))
        tail = fh.read(tail_n).decode("utf-8", errors="ignore")
    return f"{head}\n\n... [input truncated, {size} bytes total] ...\n\n{tail}"
    ...
    for rel in list(changed_paths(diff)) + list(ANCHORS):
        ...
        try:
            ...
            if not candidate.is_relative_to(root):
                continue
            text = _read_capped(candidate, per_file_cap)
...
def main() -> int:
    ...
    try:
        ...
        file_cap = 40_000
    if file_cap <= 0:                        # clamp pathological values to the default
        file_cap = 40_000
